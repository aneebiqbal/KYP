...
export default function Reviews({reviews,professorId,updateRatings}) {
  ...
  const updateReactRating = async (check,review)=>{
    let reaction = {
      ...
      reported: review.reactRatings.reported,
    }
    ...
    if(check === 1){
      ...
      reaction.reported = !review.reactRatings.reported;
    ...
    updatedReview.reactRatings.reported = reaction.reported;
    ...
    {reviews.map((review,index) => (
      <div key={'review'+index} className="flex mb-20">
        <div className="flex column items-center reviews">
          <Image className="border-radius-100" height={48} width={48} src={false?review.student_image_url:'/student.png'} alt={review.image_url} />
          <p className="text-0378A6 text-26 text-weight-600">{(review.rating).toFixed(2)}</p>
        </div>
        <div className="flex-1 ml-12  border-radius-10 pa-20 bg-E6F1F6">
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center">
              <Image className="cursor-pointer" width={16} height={16} src={review.reactRatings.upvote?'/likeTrue.png':'/like.png'} alt="like" onClick={()=>{updateReactRating(1,review)}}/>
              <p className="text-14 text-434343 text-weight-400 " style={{marginLeft:'4px',marginRight:'8px'}}>{review.upVotes}</p>
              <Image className="cursor-pointer" width={16} height={16} src={review.reactRatings.downvote?'/dislikeTrue.png':'/dislike.png'}alt="like" onClick={()=>{updateReactRating(2,review)}}/>
              <p className="text-14 text-434343 text-weight-400" style={{marginLeft:'4px',marginRight:'8px'}}>{review.downVotes}</p>
              <Image className="cursor-pointer" width={13} height={16} src={review.reactRatings.reported?'/reportTrue.png':'/report.png'} alt="like" onClick={()=>{updateReactRating(3,review)}}/>
