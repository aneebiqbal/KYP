        ...
        <section>
          <div className="container py-5">
            <div className="d-flex flex-column align-items-center mb-5">
              <Image width={200} height={200} src="/user/userImage.png" alt="userImage" className="mb-3 img-fluid" />
              <h1 className="text-weight-700 text-1F1F1F text-48 text-capitalize">{userInfo.first_name+' '+userInfo.last_name}</h1>
        <p className="text-434343 text-weight-400 text-24 mb-60">{userInfo?.institute || "School Name here"}</p>
              <div className="d-flex flex-wrap justify-content-center mt-5 gap-5">
  <p
    className={`text-141414 text-24 text-weight-400 cursor-pointer ${activeTab === 0 ? 'active-user-tab pb-2' : ''}`}
    onClick={() => setActiveTab(0)}
  >
    Saved Professors
  </p>
  <p
    className={`text-141414 text-24 text-weight-400 cursor-pointer ${activeTab === 1 ? 'active-user-tab pb-2' : ''}`}
    onClick={() => setActiveTab(1)}
  >
    Ratings
  </p>
  <p
    className={`text-141414 text-24 text-weight-400 cursor-pointer ${activeTab === 2 ? 'active-user-tab pb-2' : ''}`}
